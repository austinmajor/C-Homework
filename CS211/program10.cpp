...
// This is for type of zip code
enum zipcode_type {
	ROMAN,
	POSTNET
};

// This is for the destination of print
enum destination {
	CONSOLE,
	TEXT_FILE
};

// Class Zipcode is used for postnet and roman zipcode types
class Zipcode {
	private:
		int roman;
		...
		// Takes the roman and gets the postnet
		void setPostnet() {
		...
		// Takes the postnet and sets the roman zip code
		void setRoman() {
		...
		// Takes the postnet in 1's and 0's and turns it into a bar code
		void setBarcode() {
			if(this->postnet == "") return;
		...
		// Creates the text file the bar code will be stored in
		void CreateFile() {
			if(this->roman == 0) return;
	...
	public:
		...
		// Zipcode class closes the file stream
		~Zipcode() {
			file.close();
		}

		// Returns the private roman variable
		int GetRoman() {
			return this->roman;
		}

		// Returns the private postnet variable
		string GetPOSTNET() {
			return this->postnet;
		}

		// Takes a destination parameter and prints the barcode
...
Creates a file whose name is in the format ROMAN.txt
 */
